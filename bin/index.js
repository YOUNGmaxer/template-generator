...
// src/generator/template-generator.ts
...
function copyDirOrFile(src, dest, config) {
  const stats = import_node_fs.default.statSync(src);
  const name = import_node_path.default.basename(src);
  if (stats.isDirectory()) {
    if (name === "node_modules")
      return;
    import_node_fs.default.mkdirSync(dest, { recursive: true });
    for (const file of import_node_fs.default.readdirSync(src)) {
      copyDirOrFile(import_node_path.default.resolve(src, file), import_node_path.default.resolve(dest, file), config);
    }
  } else {
...
// src/generator/template-generator.ts
var templateRootPath = import_node_path2.default.resolve(__dirname, globalThis.__DEV__ ? "../../template" : "../template");
function buildSourcePath(part) {
  return import_node_path2.default.resolve(templateRootPath, part);
}
var beforeFileCopied = (filename, src, dest) => {
  if (filename.startsWith("_")) {
    dest = import_node_path2.default.resolve(import_node_path2.default.dirname(dest), filename.replace(/^_/, "."));
  }
  import_node_fs2.default.copyFileSync(src, dest);
  return false;
};
function generateTemplate(part, dest) {
  ...
  const response = await (0, import_prompts.default)([
    {
      ...
      format: (inputName) => inputName.trim()
    }
  ]);
